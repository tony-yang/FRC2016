...
void RobotMap::init() {
	...
	drivetrainRobotDrive41.reset(
			new RobotDrive(drivetrainLeftFrontMotor, drivetrainLeftRearMotor,
	...
	lw->AddActuator("Drivetrain", "LeftMotorSolenoid",
			drivetrainLeftMotorSolenoid);

	drivetrainRightMotorSolenoid.reset(new Solenoid(5, 1));
	lw->AddActuator("Drivetrain", "RightMotorSolenoid",
			drivetrainRightMotorSolenoid);

	scalingMotor.reset(new Talon(3));
	lw->AddActuator("Scaling", "Motor",
			std::static_pointer_cast<Talon>(scalingMotor));
	...
	lw->AddActuator("BallManipulation", "PivotMotor",
			ballManipulationPivotMotor);

	ballManipulationFetchMotor.reset(new CANTalon(11));
	lw->AddActuator("BallManipulation", "FetchMotor",
			ballManipulationFetchMotor);

	ballManipulationBallMotor21.reset(
			new RobotDrive(ballManipulationPivotMotor,
					ballManipulationFetchMotor));
